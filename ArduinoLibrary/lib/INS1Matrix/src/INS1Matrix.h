...
class INS1Matrix
{
  public:
    ...
    void setAnimationToDisplay(uint32_t **animationImgData, uint8_t displays, uint8_t frames, uint8_t delay); // write 2 uint32_t to display * the num of displays, how many frames are in the animation data, the delay between frames in ms. imgData should be an array of uint32_t 2*displays*frames big
    void animateDisplay();
